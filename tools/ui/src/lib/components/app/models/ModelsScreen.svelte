<script lang="ts">
	...
	// Deletion state
	let deleteModelId = $state<string | null>(null);
	...
	let activeDownloadsList = $derived.by(() => {
		const list: { id: string; done: number; total: number; speed: number; startTime: number }[] =
			[];
		// Automatically select newly loaded/downloaded model if there's no active model selected
		for (const [id, progress] of modelsStore.downloadProgress.entries()) {
			list.push({ id, ...progress });
			if (progress.done === progress.total && progress.total > 0 && !hasActiveModel) {
				handleModelSelect(id);
			}
		}
		// Also find any server router models showing downloading state
		for (const m of modelsStore.routerModels) {
			if (m.status.value === ServerModelStatus.DOWNLOADING && !modelsStore.downloadProgress.has(m.id)) {
	...
	function handleSseRecord(record: string) {
		...
		try {
			...
			if (event === 'download_finished' || event === 'download_failed' || event === 'models_reload') {
				void modelsStore.fetchRouterModels();
			}
		} catch (e) {
			// Parse error
...
<div class="mx-auto w-full p-4 md:p-8 md:py-8" in:fade={{ duration: 150 }}>
	...
	<div class="grid gap-6">
		...
		{#if activeTab === 'local'}
			...
			<div class="rounded-xl border border-border/50 bg-card p-6 shadow-sm">
				...
				{#if localModels.length === 0}
					<div class="py-12 px-6 flex flex-col items-center justify-center text-center border-2 border-dashed border-border/50 rounded-2xl bg-muted/10 my-4">
						<div class="h-12 w-12 rounded-full bg-primary/10 flex items-center justify-center mb-4">
							<Search class="h-6 w-6 text-primary" />
						</div>
						<h4 class="text-lg font-bold mb-2">No Models Found</h4>
						<p class="text-sm text-muted-foreground max-w-md mx-auto">
							Your local library is empty. Search Hugging Face above to find your first model, or paste a URL directly.
						</p>
					</div>
				{:else}
							...
							>
								...
								>
									...
									<Button
										...
										class="flex items-center gap-1.5 h-6 px-3 text-[10px] font-bold rounded-full text-purple-100 bg-purple-500/20 hover:bg-purple-500/30 border border-purple-500/30 transition-all hover:scale-105 active:scale-95 shadow-[0_0_10px_rgba(168,85,247,0.15)]"
										title="Automatically configure local IDE configurations to map to this model"
									>
										{#if updatingIde}
											<Loader2 class="h-3 w-3 animate-spin" />
										{:else}
											<Terminal class="h-3 w-3 text-purple-400" />
										{/if}
										UPDATE IDE
									</Button>
									...
									<div class="flex items-center gap-2 mt-1">
										...
										<span class="text-[11px] font-bold text-neutral-500 dark:text-neutral-400"
											>{estimatedRam}</span
										>
										<Button
											variant="ghost"
		...
		{:else if activeTab === 'online'}
			...
			<div class="rounded-xl border border-border/50 bg-card p-6 shadow-sm">
				...
				{:else if onlineModels.length === 0}
					<div class="py-16 flex flex-col items-center justify-center text-center bg-muted/5 rounded-xl border border-border/20 border-dashed m-2">
						...
						<h4 class="text-lg font-bold text-foreground mb-2 tracking-tight">No Online Models Found</h4>
						<p class="text-sm text-muted-foreground max-w-sm mx-auto mb-0 leading-relaxed">
							Make sure your API key is configured and saved for this provider to fetch available models.
						</p>
					</div>
				{:else}
					<div class="grid gap-3">
		...
		{:else if activeTab === 'huggingface'}
			...
			<div class="rounded-xl border border-border/50 bg-card p-6 shadow-sm">
				...
				{:else if displayedTrendingModels.length === 0}
					<div class="py-20 flex flex-col items-center justify-center text-center bg-muted/5 rounded-xl border border-border/20 border-dashed m-4">
						<div class="w-16 h-16 mb-5 rounded-full bg-background flex items-center justify-center border border-border/50 shadow-sm">
							<Search class="h-8 w-8 text-muted-foreground/60" />
						</div>
						<h4 class="text-lg font-bold text-foreground mb-2 tracking-tight">No Hugging Face Models Found</h4>
						<p class="text-sm text-muted-foreground max-w-md mx-auto mb-6 leading-relaxed">
							We couldn't find any models matching your current search parameters. Try adjusting your filters or clearing your search query.
						</p>
						<Button variant="outline" class="hover:bg-primary hover:text-primary-foreground gap-2 font-semibold tracking-wide" onclick={() => { searchQuery = ''; limit = 50; fetchTrendingModels(); }}>
							<RefreshCw class="h-4 w-4" /> RESET FILTERS
						</Button>
					</div>
				{:else}
					<div class="grid gap-3">
	...
	<DialogConfirmation
		...
		title="Delete Model"
		description="Are you sure you want to delete this model? It will be removed from your disk cache and you will need to download it again."
