...
<div class="flex flex-col gap-2 {className}">
				...
				{#each argumentEntries as [key, value] (key)}
					<Tooltip.Root>
						<Tooltip.Trigger>
							...
							<span
								class="rounded-sm bg-neutral-200/60 px-1.5 py-0.5 text-[10px] leading-none text-neutral-700 transition-opacity dark:bg-neutral-800/40 dark:text-neutral-300 {hoveredArgKey &&
								hoveredArgKey !== key
									? 'opacity-30'
									: ''}"
	...
	{:else if isLoading}
		<Card
			class="relative overflow-hidden rounded-[1.125rem] border border-neutral-200 bg-neutral-500/5 px-1 py-2 backdrop-blur-md dark:border-neutral-800 dark:bg-neutral-500/10"
		>
			<div
				class="overflow-y-auto {paddingClass}"
				style="{maxHeightStyle} overflow-wrap: anywhere; word-break: break-word;"
	...
	{:else if hasContent}
		<Card
			class="relative overflow-hidden rounded-[1.125rem] border border-neutral-200 bg-neutral-500/5 py-0 text-foreground backdrop-blur-md dark:border-neutral-800 dark:bg-neutral-500/10"
		>
			<div
				class="overflow-y-auto {paddingClass}"
				style="{maxHeightStyle} overflow-wrap: anywhere; word-break: break-word;"
			>
				<span class="{textSizeClass} whitespace-pre-wrap">
					...
					{#each contentParts as part, i (i)}{#if part.argKey}<span
								class="rounded-sm bg-neutral-300/50 px-0.5 text-neutral-900 transition-opacity dark:bg-neutral-700/50 dark:text-neutral-100 {hoveredArgKey &&
