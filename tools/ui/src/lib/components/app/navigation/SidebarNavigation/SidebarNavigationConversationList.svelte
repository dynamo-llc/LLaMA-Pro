...
{:else}
	...
	<div class="mt-2 flex min-h-0 flex-col gap-4 md:gap-2 whitespace-nowrap {className}">
		...
		<div class="min-h-0 md:overflow-y-auto">
			<ul class="flex w-full min-w-0 flex-col gap-4 md:gap-1">
				{#each unpinnedConversations.slice(0, 4) as { conversation, depth } (conversation.id)}
					<li class="group/item relative mb-1 p-0">
						<SidebarNavigationConversationItem
							conversation={{
								id: conversation.id,
